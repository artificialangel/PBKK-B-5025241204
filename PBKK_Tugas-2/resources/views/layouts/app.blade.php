    ...
    <header id="site-header" class="fixed top-0 left-0 right-0 z-20 transition-colors duration-300 bg-transparent">
        <nav class="max-w-7xl mx-auto flex items-center justify-between px-4 sm:px-8 py-4 sm:py-6">
            ...
            <div class="flex items-center gap-3 sm:gap-10 text-xs sm:text-sm">
                <a href="{{ route('home') }}" class="nav-link hover:text-gray-300 transition {{ request()->routeIs('home') ? 'text-white' : 'text-gray-300' }}">Home</a>
                <a href="{{ route('about') }}" class="nav-link hover:text-gray-300 transition {{ request()->routeIs('about') ? 'text-white' : 'text-gray-300' }}">About</a>
                <a href="{{ route('calculator') }}" class="nav-link hover:text-gray-300 transition {{ request()->routeIs('calculator') || request()->routeIs('hitung.ipk') ? 'text-white' : 'text-gray-300' }}">Calculator</a>
                <a href="{{ route('agent') }}" class="nav-link hover:text-gray-300 transition {{ request()->routeIs('agent') ? 'text-white' : 'text-gray-300' }}">Agent</a>
            </div>
        </nav>
    </header>
    ...
    <footer class="relative z-20 bg-black px-8 py-6">
        <div class="max-w-7xl mx-auto flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 text-sm text-gray-400">
            ...
            <div class="text-left sm:text-right">
                <a href="https://github.com/artificialangel/PBKK-B-5025241204"
                    target="_blank"
                    class="text-white hover:underline">
                    Github
                </a>
                <p>5025241204@student.its.ac.id</p>
            </div>
        </div>
    </footer>
